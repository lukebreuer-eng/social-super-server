-- Social Super Server — SQLite Views voor Rapportage
-- Uitvoeren via: sqlite3 /path/to/database.db < create-sql-views.sql
-- Of via Coolify terminal in de Directus container
...
SELECT
  ...
  CASE
    ...
    THEN ROUND((CAST(p.engagement_likes + p.engagement_comments + p.engagement_shares AS REAL) / p.engagement_reach) * 100, 2)
    ELSE 0
...
GROUP BY l.bedrijf, l.lead_temperature, l.status;
...
SELECT
  ...
  SUM(CASE WHEN p.ai_generated = 1 THEN 1 ELSE 0 END) AS ai_gegenereerd,
  ROUND(AVG(CASE WHEN p.seo_score > 0 THEN p.seo_score END), 1) AS avg_seo_score,
...
LEFT JOIN Bedrijven b ON p.bedrijf = b.id
WHERE p.published_at IS NOT NULL
ORDER BY p.engagement_score DESC, p.engagement_reach DESC
LIMIT 50;
...
SELECT
  ...
  ROUND(AVG(CASE WHEN p.seo_score > 0 THEN p.seo_score END), 1) AS avg_score,
